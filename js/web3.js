...
async function l3gCopyText(text, label) {

    const successMsg = label || "Address copied";

    try {

        if (navigator.clipboard && window.isSecureContext) {
            await navigator.clipboard.writeText(text);
            l3gNotify(successMsg, "success");
            return;
        }
        ...
        // Fallback for older/insecure-context browsers without the
        // async Clipboard API (some older Android WebViews, http pages).
        try {
            ...
            l3gNotify("Could not copy — long-press to copy manually", "error");
        }
...
function l3gIsMobile() {
    ...
    // iPadOS 13+ reports a desktop Safari UA on newer iPads — detect it
    // via touch support instead, otherwise these devices silently fall
    // through to the desktop "install the extension" flow.
    if (/iPad/i.test(ua)) return true;
    if (/Macintosh/i.test(ua) && navigator.maxTouchPoints > 1) return true;

    // Fallback heuristic for any other touch device (foldables, newer
    // Android tablets, etc.) with a phone/tablet-sized viewport.
    if ("ontouchstart" in window && window.innerWidth <= 992) return true;

    return false;

}

function l3gPlatform() {
...
async function l3gConnectWallet() {
    ...
    if (typeof ethers === "undefined") {
        l3gNotify("Wallet library failed to load — check your connection and refresh", "error");
        return;
    }
...
// If the deep link above doesn't have the wallet app installed, the OS
// either does nothing or shows its own "can't open" prompt — the page
// stays visible and in the foreground. We use that to fall back to the
// correct app store listing automatically.
function l3gWalletStoreLink(type) {

    const platform = l3gPlatform();
    ...
    const stores = {
        ...
        okx: {
            ios: "https://apps.apple.com/app/okx-buy-bitcoin-btc-eth/id1327268470",
            android: "https://play.google.com/store/apps/details?id=com.okinc.okex.gp"
        },
        tokenpocket: {
            ios: "https://apps.apple.com/app/tokenpocket-crypto-defi-wallet/id1436028697",
            android: "https://play.google.com/store/apps/details?id=vip.mytokenpocket"
        }
    };

    const entry = stores[type] || stores.metamask;
    return entry[l3gPlatform()] || entry.android;
...
function l3gConnectMobileWallet(type) {
    ...
    if (type === "walletconnect") {
        ...
        // No WalletConnect Cloud project is configured on this build, so a
        // real WC pairing modal/QR isn't available. The reliable fallback
        // that works for every wallet is opening the site inside the
        // wallet's own built-in browser, where it injects window.ethereum
        // itself — so we copy the link for the user to paste there.
        l3gCopyText(window.location.href, "Link copied — paste it into your wallet app's built-in browser to connect");
        l3gNotify("Open your wallet app's browser and paste the copied link", "info");
        return;

    }

    // Try to open the wallet app directly. If it isn't installed, nothing
    // will happen and the page stays in the foreground — after a short
    // delay, send the user to the correct app store instead of leaving
    // them stuck on a dead link.
    const fallbackTimer = setTimeout(() => {
        if (document.visibilityState === "visible") {
            window.location.href = l3gWalletStoreLink(type);
        }
    }, 1500);

    window.addEventListener("visibilitychange", function onVisChange() {
